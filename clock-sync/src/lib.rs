...
//!
//! The fine loop provides steady-state synchronization. A stake-weighted
//! panic loop coordinates recovery when the fine loop loses quorum. The
//! service remains in shadow mode: nothing consensus-facing reads the
//! synchronized clock.
...
/// d: retransmission interval for panic recovery.
pub const PANIC_INTERVAL: Duration = Duration::from_millis(200);

/// Panics older than this do not contribute to a recovery quorum.
pub const PANIC_OBSERVATION_TTL: Duration = Duration::from_millis(800);

/// Sustained inbound datagrams-per-second each peer may send us; the
/// protocol rate is one pulse plus at most five panic messages per second.
pub const CLOCK_SYNC_RATE_LIMIT_PPS: usize = 8;
